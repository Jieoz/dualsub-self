...
test("restoreAndPackTokens 对已验证超长句做一次局部 clause rescue", async () => {
  ...
  ], "短于最小屏长的 trailing adjunct 应留在前屏，且不能退化成超长单屏");
  assert.ok(units.every((u) => u.content.split(/\s+/).length <= 16), "每个真实显示单元必须保持可读行长");
...
test("restoreAndPackTokens 真实水壶长句拆成三个可读屏且保留词与时间", async () => {
  const source = "let me reiterate that the cheapest electric kettle I could get my hands on is significantly faster at boiling water than this stove top kettle despite being limited by our 120 volt electrical system";
  ...
  assert.ok(units.every((u) => u.content.split(/\s+/).length <= 16), "真实长句不得再退化为 20–34 词单屏");
  assert.strictEqual(units.map((u) => u.content).join(" "), source, "分屏必须逐词保真");
  assert.deepStrictEqual(units.map((u) => [u.start, u.end]), [[237505, 243105], [243105, 247505], [247505, 251105]], "每屏时间必须直接来自边界 token");
});
...
test("makeCacheKey v0.5.9 隔离旧缓存、无句号输出与 fallback/semantic 分段", () => {
  ...
  assert.ok(fallback.startsWith("dsc-v60|fallback|"), "双语严格单行与英文自然分屏必须隔离旧缓存");
