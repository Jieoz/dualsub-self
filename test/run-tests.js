...
test("DEFAULT_SYSTEM_PROMPT：完整语义单元 1:1（完整译文/标点/不切词）", () => {
  ...
  assert.ok(/标点|句号/.test(filled) && /问号|感叹号|句号/.test(filled), "应含必要标点规则");
  assert.ok(/每条都必须在本屏收束/.test(filled) && /绝不能以逗号/.test(filled), "连续字幕片段也必须在本屏自然收束");
...
test("buildSrt：兼容 isolated.js 的 start/end 命名", () => {
  ...
  test("语义恢复不阻塞 fallback 首屏，切换时隔离旧异步结果与缓存", () => {
    ...
    assert.ok(/"dsc-v59"/.test(fs.readFileSync(path.join(ROOT, "core.js"), "utf8")), "双语严格单行与英文自然分屏必须升级缓存 namespace");
  ...
  test("DEFAULT_SYSTEM_PROMPT 强调上下文 1:1 与双语单行", () => {
    ...
    assert.ok(/每条都必须在本屏收束/.test(p) && /绝不能以逗号/.test(p), "应要求每屏以完整中文收束");
  });
