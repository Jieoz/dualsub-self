...
test("filterUnsafeRescueMarks 只删除比较从句的坏边界，保留前面的自然分屏", () => {
  ...
  marks[13] = "|"; // hands on | is ...
  marks[19] = "|"; // water | than this ... (bad)
  const filtered = Core.filterUnsafeRescueMarks(words, marks);
  assert.strictEqual(filtered[13], "|");
  assert.strictEqual(filtered[19], "");
});
