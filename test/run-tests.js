...
test("segment：长译文(>maxChars)切成 N>=2 段，每段不以半个词/数字结尾(切点在标点)", () => {
  ...
  // 除末段外，每段都应以标点结尾（切点只在标点处，绝不拦腰斩词/数字）
  const punct = /[。！？．.!?；;，,、…]$/;
  for (let i = 0; i < out.length - 1; i++) {
    assert.ok(punct.test(out[i].translation), "第" + i + "段应以标点结尾: " + out[i].translation);
  }
  // 绝不切断 "Transformer"：不应有任何段以 "Transforme" 结尾或以 "r" 开头这类碎片
  for (const u of out) {
    assert.ok(!/Transforme$/.test(u.translation), "不应把 Transformer 拦腰斩断");
  }
});
...
test("segment：各段译文拼回无丢字", () => {
  ...
  assert.strictEqual(joined, Core.collapseWhitespace(trans), "拼接译文应等于原译文（无丢字）");
});
...
test("segment：无标点长译文不硬切词，原样返回单段", () => {
  const unit = {
    startMs: 0,
    endMs: 6000,
  ...
  assert.strictEqual(out.length, 1, "无标点 → 不拦腰斩词，整段返回");
  assert.strictEqual(out[0].translation, unit.translation);
});
