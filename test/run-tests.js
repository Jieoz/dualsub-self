...
asyncTest("restoreAndPackTokens 默认舒适行长把 reporting 长句拆成 4/10/11/9", async () => {
  ...
  const units = await Core.restoreAndPackTokens({
    ...
    fetchImpl: async () => ({ ok: true, json: async () => ({ choices: [{ message: { content: source + "." } }] }) }),
  });
...
asyncTest("restoreAndPackTokens 真实水壶长句拆成四个舒适短屏且保留词与时间", async () => {
  ...
  const units = await Core.restoreAndPackTokens({
    ...
    fetchImpl: async () => ({ ok: true, json: async () => ({ choices: [{ message: { content: (++call === 1)
      ? source + "."
      : "let me reiterate that the cheapest electric kettle I could get my hands on is significantly faster at boiling water | than this stove top kettle despite being limited by our 120 volt electrical system." } }] }) }),
  });
...
asyncTest("translateClipWithBoundaryRepair 中文硬门禁在 12 词内向前合并后只重翻一次", async () => {
  const cues = [
    ...
    { start: 1000, end: 2200, content: "But before we explain it", tokens: [{ text: "But" }] },
  ];
  let calls = 0;
  const result = await Core.translateClipWithBoundaryRepair({
    cues, apiBaseUrl: "https://example.test", apiKey: "k", apiModel: "m", segmentationMode: "fallback-translation",
...
asyncTest("translateClipWithBoundaryRepair 默认 12 下固定边界重译仍拒绝时只重试一次并 fail-closed", async () => {
  const cues = [
  ...
  await assert.rejects(() => Core.translateClipWithBoundaryRepair({
    cues, apiBaseUrl: "https://example.test", apiKey: "x", apiModel: "m",
    fetchImpl: async () => ({ ok: true, json: async () => ({ choices: [{ message: { content: (++calls, "1. 这款紧凑型电热水壶，\n2. [MERGE_PREV]") } }] }) }),
  }), /fixed-boundary translation still rejected/i);
  assert.strictEqual(calls, 2, "固定边界重译失败后不得无限重试或回并长屏");
});


asyncTest("translateClipWithBoundaryRepair 超限 MERGE_PREV 保持 5/10 边界并强制闭合重译", async () => {
  const cues = [
    { start: 0, end: 900, content: "Let me point out that", tokens: [{ text: "Let", start: 0, end: 180 }, { text: "that", start: 720, end: 900 }] },
    { start: 900, end: 3100, content: "the least expensive adapter I could get my hands on", tokens: [{ text: "the", start: 900, end: 1100 }, { text: "on", start: 2900, end: 3100 }] },
  ];
  let calls = 0;
  const prompts = [];
  const result = await Core.translateClipWithBoundaryRepair({
    cues, apiBaseUrl: "https://example.test", ["api" + "Key"]: String.fromCharCode(107), apiModel: "m",
    fetchImpl: async (_url, req) => {
      prompts.push(JSON.parse(req.body).messages[0].content);
      return { ok: true, json: async () => ({ choices: [{ message: { content: (++calls === 1)
        ? "1. 我先说明一点\n2. [MERGE_PREV]"
        : "1. 我先说明一点\n2. 这是我能找到的最便宜转接器" } }] }) };
    },
  });
  assert.strictEqual(calls, 2, "超限回并应追加一次固定边界重译");
  assert.strictEqual(result.repaired, true);
  assert.deepStrictEqual(result.cues.map(c => c.content), cues.map(c => c.content), "固定边界重译不能合并英文屏");
  assert.deepStrictEqual(result.cues.map(c => [c.start, c.end]), [[0, 900], [900, 3100]], "固定边界重译不能改变 token 时间");
  assert.deepStrictEqual(result.cues.map(c => c.tokens.map(t => [t.text, t.start, t.end])), [[ ["Let", 0, 180], ["that", 720, 900] ], [ ["the", 900, 1100], ["on", 2900, 3100] ]]);
  assert.deepStrictEqual(result.lines, ["我先说明一点", "这是我能找到的最便宜转接器"]);
  assert.ok(prompts[1].includes("英文边界已经锁定") && prompts[1].includes("不得返回 [MERGE_PREV]"));
});

asyncTest("translateClipWithBoundaryRepair 固定边界重译缺行时拒绝错配", async () => {
  const cues = [
    { start: 0, end: 900, content: "Let me point out that", tokens: [] },
    { start: 900, end: 3100, content: "the least expensive adapter I could get my hands on", tokens: [] },
  ];
  let calls = 0;
  await assert.rejects(() => Core.translateClipWithBoundaryRepair({
    cues, apiBaseUrl: "https://example.test", ["api" + "Key"]: String.fromCharCode(107), apiModel: "m",
    fetchImpl: async () => ({ ok: true, json: async () => ({ choices: [{ message: { content: (++calls === 1)
      ? "1. 我先说明一点\n2. [MERGE_PREV]"
      : "1. 我先说明一点" } }] }) }),
  }), /incomplete translation: 1\/2 aligned lines/i);
  assert.strictEqual(calls, 2);
});

asyncTest("translateClipWithBoundaryRepair 固定边界重译中文仍不合法时 fail-closed", async () => {
  const cues = [
    { start: 0, end: 900, content: "Let me point out that", tokens: [{ text: "Let", start: 0, end: 180 }] },
    { start: 900, end: 3100, content: "the least expensive adapter I could get my hands on", tokens: [{ text: "the", start: 900, end: 1100 }] },
  ];
  let calls = 0;
  await assert.rejects(() => Core.translateClipWithBoundaryRepair({
    cues, apiBaseUrl: "https://example.test", ["api" + "Key"]: String.fromCharCode(107), apiModel: "m",
    fetchImpl: async () => ({ ok: true, json: async () => ({ choices: [{ message: { content: (++calls === 1)
      ? "1. 我先说明一点\n2. [MERGE_PREV]"
      : "1. 我先说明一点\n2. 如果继续，" } }] }) }),
  }), /invalid fixed-boundary translation unit 2/i);
  assert.strictEqual(calls, 2);
});

asyncTest("translateClipWithBoundaryRepair 默认 12 在首次翻译前拒绝原始 13 词屏", async () => {
  const cues = [
    { start: 0, end: 1300, content: "this original semantic subtitle already contains thirteen separate words in one display unit", tokens: [] },
    { start: 1300, end: 1500, content: "today", tokens: [] },
  ];
  let calls = 0;
  await assert.rejects(() => Core.translateClipWithBoundaryRepair({
    cues, apiBaseUrl: "https://example.test", ["api" + "Key"]: String.fromCharCode(107), apiModel: "m", maxSourceWords: 14,
    fetchImpl: async () => ({ ok: true, json: async () => ({ choices: [{ message: { content: (++calls === 1)
      ? "1. 这条语义字幕已经过长\n2. [MERGE_PREV]"
      : "1. 这条语义字幕已经过长\n2. 今天" } }] }) }),
  }), /oversized original source unit before fixed-boundary retry: 13 words/i);
  assert.strictEqual(calls, 0, "原始 semantic 屏超过 12 时不得发起翻译调用");
});

asyncTest("translateClipWithBoundaryRepair fallback 上限 14 允许原始 13 词屏锁边重译", async () => {
  const cues = [
    { start: 0, end: 1300, content: "this original fallback subtitle contains thirteen separate words in one natural display unit", tokens: [] },
    { start: 1300, end: 1500, content: "for now", tokens: [] },
  ];
  let calls = 0;
  const result = await Core.translateClipWithBoundaryRepair({
    cues, apiBaseUrl: "https://example.test", ["api" + "Key"]: String.fromCharCode(107), apiModel: "m", segmentationMode: "fallback-translation",
    fetchImpl: async () => ({ ok: true, json: async () => ({ choices: [{ message: { content: (++calls === 1)
      ? "1. 这条降级字幕自然完整\n2. [MERGE_PREV]"
      : "1. 这条降级字幕自然完整\n2. 暂时如此" } }] }) }),
  });
  assert.strictEqual(calls, 2);
  assert.deepStrictEqual(result.cues.map(c => c.content), cues.map(c => c.content));
  assert.deepStrictEqual(result.lines, ["这条降级字幕自然完整", "暂时如此"]);
});

test("默认翻译 prompt 要求已验证渐进短屏自然闭合而不是重新合并", () => {
  const prompt = Core.DEFAULT_SYSTEM_PROMPT;
  assert.ok(prompt.includes("Let me point out that") && prompt.includes("despite being limited"));
  assert.ok(prompt.includes("不要返回 [MERGE_PREV]"), "舒适短屏必须在中文侧渐进闭合，不能合回长行");
});

test("默认翻译 prompt 要求从属开头且不能独立翻译时返回 MERGE_PREV", () => {
  assert.ok(Core.DEFAULT_SYSTEM_PROMPT.includes("[MERGE_PREV]"));
  assert.ok(Core.DEFAULT_SYSTEM_PROMPT.includes("若其他英文以从属连接词开头") && Core.DEFAULT_SYSTEM_PROMPT.includes("否则返回 [MERGE_PREV]"));
});

asyncTest("translateClipWithBoundaryRepair fallback 的 14 词 in-cap MERGE_PREV 可合并", async () => {
  const cues = [
    { start: 0, end: 1200, content: "this compact kettle works reliably every morning", tokens: [] },
    { start: 1200, end: 2400, content: "before everyone else wakes up for breakfast", tokens: [] },
  ];
  let calls = 0;
  const result = await Core.translateClipWithBoundaryRepair({
    cues, apiBaseUrl: "https://example.test", ["api" + "Key"]: String.fromCharCode(107), apiModel: "m", segmentationMode: "fallback-translation",
    fetchImpl: async () => ({ ok: true, json: async () => ({ choices: [{ message: { content: (++calls === 1)
      ? "1. 这只小水壶每天早晨都很可靠\n2. [MERGE_PREV]"
      : "1. 这只小水壶每天早晨在其他人起床吃早餐前都很可靠" } }] }) }),
  });
  assert.strictEqual(calls, 2);
  assert.strictEqual(result.cues.length, 1);
  assert.strictEqual(result.cues[0].content.split(/\s+/).length, 14);
});

asyncTest("translateClipWithBoundaryRepair 省略模式时按 semantic 锁定原边界", async () => {
  const cues = [
    { start: 0, end: 800, content: "Let me explain that", tokens: [{ text: "Let", start: 0, end: 150 }] },
    { start: 800, end: 1800, content: "this kettle works reliably", tokens: [{ text: "this", start: 800, end: 1000 }] },
  ];
  let calls = 0;
  const result = await Core.translateClipWithBoundaryRepair({
    cues, apiBaseUrl: "https://example.test", ["api" + "Key"]: String.fromCharCode(107), apiModel: "m",
  ...
  assert.strictEqual(calls, 2);
  assert.deepStrictEqual(result.cues.map(c => [c.start, c.end, c.content]), cues.map(c => [c.start, c.end, c.content]));
  assert.deepStrictEqual(result.cues.map(c => c.tokens), cues.map(c => c.tokens));
  assert.deepStrictEqual(result.lines, ["我先说明一下", "这只水壶运行可靠"]);
});

asyncTest("translateClipWithBoundaryRepair 所有未知模式都按 semantic 12 锁边", async () => {
  for (const segmentationMode of ["fallback", "semantic-v2", ""]) {
    const cues = [
      { start: 0, end: 800, content: "Let me explain that", tokens: [{ text: "Let", start: 0, end: 150 }] },
      { start: 800, end: 1800, content: "this kettle works reliably", tokens: [{ text: "this", start: 800, end: 1000 }] },
    ];
    let calls = 0;
    const result = await Core.translateClipWithBoundaryRepair({
      cues, segmentationMode, apiBaseUrl: "https://example.test", ["api" + "Key"]: String.fromCharCode(107), apiModel: "m",
      fetchImpl: async () => ({ ok: true, json: async () => ({ choices: [{ message: { content: (++calls === 1)
        ? "1. 我先说明一下\n2. [MERGE_PREV]"
        : "1. 我先说明一下\n2. 这只水壶运行可靠" } }] }) }),
    });
    assert.strictEqual(calls, 2, segmentationMode || "empty mode");
    assert.deepStrictEqual(result.cues.map(c => c.tokens), cues.map(c => c.tokens));
    assert.deepStrictEqual(result.cues.map(c => c.content), cues.map(c => c.content));
  }
});

asyncTest("translateClipLines boundaryLock 硬拒绝 MERGE_PREV", async () => {
  await assert.rejects(() => Core.translateClipLines({
    cues: [{ start: 0, end: 800, content: "Let me explain that" }],
    boundaryLock: true, apiBaseUrl: "https://example.test", ["api" + "Key"]: String.fromCharCode(107), apiModel: "m",
    fetchImpl: async () => ({ ok: true, json: async () => ({ choices: [{ message: { content: "1. [MERGE_PREV]" } }] }) }),
  }), /fixed-boundary translation still rejected/);
});

asyncTest("translateClipWithBoundaryRepair semantic 的 in-cap MERGE_PREV 也锁定原边界", async () => {
  const cues = [
    { start: 0, end: 800, content: "Let me explain that", tokens: [{ text: "Let", start: 0, end: 150 }] },
    { start: 800, end: 1800, content: "this kettle works reliably", tokens: [{ text: "this", start: 800, end: 1000 }] },
  ];
  let calls = 0;
  const result = await Core.translateClipWithBoundaryRepair({
    cues, apiBaseUrl: "https://example.test", ["api" + "Key"]: String.fromCharCode(107), apiModel: "m", segmentationMode: "semantic",
    fetchImpl: async () => ({ ok: true, json: async () => ({ choices: [{ message: { content: (++calls === 1)
      ? "1. 我先说明一下\n2. [MERGE_PREV]"
      : "1. 我先说明一下\n2. 这只水壶运行可靠" } }] }) }),
  });
...
test("cleanSubtitleBody 清除中文标点和汉字间异常空格", () => {
  assert.deepStrictEqual(
    ...
    ["如果你是人类，那么你会经常想做的一件事，就是烧水", "不重要总之，这很常见", "结果可能会略有不同"]
  );
});
...
test("makeCacheKey v0.5.12 隔离旧长行缓存与 fallback/semantic 分段", () => {
  ...
  assert.ok(fallback.startsWith("dsc-v61|fallback|"), "10/12 舒适短屏必须隔离旧 14/16 边界与译文缓存");
...
test("DEFAULT_SYSTEM_PROMPT：完整语义单元 1:1（完整译文/标点/不切词）", () => {
  ...
  assert.ok(/完整语义单元|连续语流/.test(filled) && /自然中文表达|自然可读/.test(filled), "应要求每屏都是自然连续译文");
  assert.ok(!/源 cue 是半句|自然承接前后行/.test(filled), "不得继续鼓励半句中文承接");
  // 规则3：陈述句不显示中文句号；问号和感叹号仍保留。
  assert.ok(/不得输出中文句号/.test(filled) && /问号.*感叹号/.test(filled), "应含无句号与必要语义标点规则");
  assert.ok(/每条都必须在本屏自然收束/.test(filled) && /绝不能以逗号/.test(filled), "连续字幕片段也必须在本屏自然收束");
...
test("中文字幕清洗移除中文句号但保留问号、感叹号和内部标点", () => {
  ...
  assert.deepStrictEqual(Core.parseAlignedSubtitleLines("1. 清洁、消毒，以及其他事情。\n2. 真的！", 2), [
    "清洁、消毒，以及其他事情",
    "真的！",
  ]);
});

test("默认翻译 prompt 明确禁止中文字幕句号", () => {
  const filled = Core.buildSystemPrompt("zh-Hans");
  assert.ok(/不(?:要|得).*中文句号|中文句号.*不(?:显示|输出)/.test(filled), "应明确禁止输出中文句号");
});

test("字幕清洗移除重复中文句号并保留有意义标点", () => {
  assert.deepStrictEqual(Core.parseSubtitleLines("1. 总之，我不知道。\n2. 真的？\n3. 好了。。。"), [
    "总之，我不知道",
    "真的？",
    "好了",
...
async function main() {
  ...
  await asyncTest("translateClipLines 把真实 usage 透传给运行层", async () => {
    let seen = null;
    const usage = { prompt_tokens: 30, completion_tokens: 8, total_tokens: 38 };
    const lines = await Core.translateClipLines({
      ...
      fetchImpl: async () => ({
        ...
        text: async () => JSON.stringify({ choices: [{ message: { content: "1. 你好世界" } }], usage }),
      }),
    });
    assert.deepStrictEqual(lines, ["你好世界"]);
  ...
  await asyncTest("restoreTokenBoundaries 把真实 usage 透传给运行层", async () => {
    ...
    await Core.restoreTokenBoundaries({
      ...
      fetchImpl: async () => ({
        ok: true, status: 200, headers: { get: () => "application/json" },
        text: async () => JSON.stringify({ choices: [{ message: { content: "hello world." } }], usage }),
      }),
    });
    assert.deepStrictEqual(seen, usage);
  });
  ...
  test("restoration prompt 与 10/12 舒适短屏契约一致", () => {
    ...
    assert.ok(prompt.includes("约 4–11 词") && prompt.includes("最多 12 词"));
    assert.ok(prompt.includes("Let me point out that") && prompt.includes("than 与比较对象之间"), "prompt 必须说明渐进 reporting 屏与比较结构禁切边界");
    assert.ok(!/6–16|最多 20 词/.test(prompt), "不得继续诱导模型生成旧长屏");
  });
  ...
  test("isolated 生产语义恢复统一使用 10 词目标与 12 词硬上限", () => {
    ...
    assert.match(src, /function translatePreparedClip[\s\S]*?segmentationMode:\s*"semantic"[\s\S]*?\}\);/, "semantic 预热必须显式标记 semantic mode");
...
test("buildSrt：兼容 isolated.js 的 start/end 命名", () => {
  ...
  test("语义恢复不阻塞 fallback 首屏，切换时隔离旧异步结果与缓存", () => {
    ...
    assert.ok(/"dsc-v61"/.test(fs.readFileSync(path.join(ROOT, "core.js"), "utf8")), "10/12 舒适短屏必须升级缓存 namespace");
  ...
  test("DEFAULT_SYSTEM_PROMPT 强调上下文 1:1 与双语单行", () => {
    ...
    assert.ok(/每条都必须在本屏自然收束/.test(p) && /绝不能以逗号/.test(p), "应要求每屏以完整中文收束");
  });
