...
(async () => {
  ...
  for (const item of corpus) {
    ...
    try {
      const cues = await Core.restoreAndPackTokens({ tokens, apiBaseUrl: base, apiKey: key, apiModel: model, reasoningEffort: "low", preferredMaxWords: 14, maxWords: 16, attempts: 2, timeoutMs: 60000 });
      if (cues.some(c => c.content.split(/\s+/).length > 16)) throw new Error("oversized cue escaped");
