...
### 显示
字号（基准值，全屏/影院模式会按播放器高度同比放大，详见下）、字重、字体族（留空=内置默认；仅用本地/系统已安装字体，不远程加载）、底部间距、原文颜色、译文颜色、描边、阴影、背景框、双语顺序（译文在上 / 原文在上）、是否显示原文行、译文未到时是否显示"翻译中…"。
...
- **更早预取**：进入某 clip 就启动下一个 clip 的预取；拖动进度条跳转停稳后立即翻目标位置所在 clip。预取已与渲染**解耦**（独立 ~1.5s 低频循环），播放位置没明显移动不重复跑昂贵逻辑。
...
- **预取与渲染解耦**：预取下一 clip 单独降频到 ~1.5s 检查一次，不再绑在高频渲染 tick 上。
