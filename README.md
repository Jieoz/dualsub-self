...
6. 每个字幕单元直接沿用对应 semantic unit 的英文与起止时间；中文 16 字以内保持单行，超过后只在安全短语边界换为最多两行，每行不超过 16 字，绝不按字符硬切词。
7. 缓存 namespace 升级为 `dsc-v55`，并按 `fallback` / `semantic` 分段模式隔离，避免旧 prompt、旧 cue 分段或不同时间轴复用错误译文；本次语义单元与 16 字排版契约也不会误读旧缓存。
...
9. 英文单元严格不超过 24 词。自然句界修复只会在此上限内回并相邻原词单元，绝不为消除悬空尾制造新的长字幕墙；这一层不请求模型、不改写、不重排，也不丢词。
...
- **译文 1:1 对齐**：模型按编号返回与完整 semantic unit 等量的译文，每个中英文单元共享同一起止时间；中文默认单行，超过 16 字才自然换为最多两行。`maxCharsPerScreen` / `maxDurPerScreen` 仍仅用于旧配置 round-trip。
