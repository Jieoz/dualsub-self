...
- **长句分屏（`maxCharsPerScreen` 默认 20 / `maxDurPerScreen` 默认 4000ms）**：超过单屏字数或时长的长译文，由 `core.js` 的 `segmentSentenceUnit` 切成 N≥2 段做分屏滚动（到点切下一段）。`maxCharsPerScreen` 是**硬上限**——标点是优选切点，但当某个标点片段本身就超上限时（中文口语 ASR 翻译常无句中标点），会在片段内部按可读边界继续切：**CJK 按字断、拉丁单词与数字（含 `1.8` / `v2.0` 这类小数版本号）视为整体绝不拦腰斩断**，保证**每一段都 ≤ 上限**（修复了「标点稀疏时第一段仍过长 / 无标点长句完全不切」）。段长**尽量均摊**(消除「前满后碎」尾巴),并按**目标阅读速度**(`TARGET_CPS`≈7 字/秒)给每屏分配「该显示多久」——剩余时间**留白成句间停顿**而非把字幕拖满到下屏(修复长句「慢飘」:从全程 ~3.4 字/秒提到稳定 ~7 字/秒、屏间出现可呼吸的 gap),每屏可视时长有 `SEG_MIN_VISIBLE_MS` 地板兜底(不一闪而过)。原文同步切成同段数、每段非空对齐。冲突时优先级:**不丢字 > 不超 `maxCharsPerScreen` > 可视时长地板 > 目标阅读速度均匀**。屏起点单调贴语音、各屏不重叠;短句原样返回单段。`maxCharsPerScreen ≤ 0` 或设极大值 = 关闭分段(向后兼容)。
