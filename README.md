...
- **长句分屏（`maxCharsPerScreen` 默认 20 / `maxDurPerScreen` 默认 4000ms）**：超过单屏字数或时长的长译文，由 `core.js` 的 `segmentSentenceUnit` 切成 N≥2 段做分屏滚动（到点切下一段）。`maxCharsPerScreen` 是**硬上限**——标点是优选切点，但当某个标点片段本身就超上限时（中文口语 ASR 翻译常无句中标点），会在片段内部按可读边界继续切：**CJK 按字断、拉丁单词与数字（含 `1.8` / `v2.0` 这类小数版本号）视为整体绝不拦腰斩断**，保证**每一段都 ≤ 上限**（修复了「标点稀疏时第一段仍过长 / 无标点长句完全不切」）。时间轴按各段字符占比线性分配，段与段连续、不重叠、完整覆盖原区间。短句原样返回单段。`maxCharsPerScreen ≤ 0` 或设极大值 = 关闭分段（向后兼容）。
