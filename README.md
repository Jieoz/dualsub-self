...
当前正式版：**v0.5.10**。可从 [GitHub Releases](https://github.com/Jieoz/dualsub-self/releases/tag/v0.5.10) 下载 Chrome MV3 安装包。
...
- 若提示“API 返回 HTML 而不是 JSON”，说明请求落到了控制台网页、网站首页、登录/WAF 页面或错误的反代路径，而不是 OpenAI 兼容 JSON API。请优先确认 Base URL 是否应以 `/v1` 结尾；v0.5.10 会显示脱敏后的 HTTP 状态、Content-Type、响应路径及是否发生重定向，不再暴露 `Unexpected token '<'` 这类无操作意义的解析错误。
...
## 翻译质量：完整双语语义单元、上下文感知且严格 1:1

主路径先为带可靠词级时间的 YouTube JSON3 自动字幕恢复自然英文句界，再保持“完整英文语义单元 / 完整自然中文 / 同一起止时间”严格 1:1：

1. fallback 技术 cue 只用于立即显示英文原文，不再送翻译 API，避免产生约六字一断的碎中文。
2. semantic 时间线把完整英文语义单元按编号一次送入 `/chat/completions`；第 N 条中文必须完整、自然地表达第 N 个英文单元，不得悬空、截断、跨编号搬运或补意。
...
7. 缓存 namespace 升级为 `dsc-v60`，并按 `fallback` / `semantic` 分段模式隔离，避免复用 v0.5.8 及更早版本中含中文句号的旧译文、旧 prompt、旧 cue 分段或不同时间轴。
8. 首屏不再等待整轨语义恢复：先安装只显示英文原文的 fallback 时间轴，语义恢复在后台完成；fallback 技术 cue 不发翻译请求。semantic 时间轴会先预热实时播放位置对应的译文，且每次异步等待后重新核对播放头；只有当前段已经准备好时才原子切换，避免屏上中文闪回“翻译中…”或因 seek / 跨段使用过期结果。
9. 英文完整句超过约 14 词时会寻找两侧均可独立翻译的自然从句边界，单屏硬上限 16 词；不在短语、比较结构、关系从句主谓、介词续接（含 `throughout`）或短语动词内部硬切。没有可信自然边界时显式回退，且不改写、不重排、不丢词。

首包默认最多等待 8 秒：扩展只会暂停自己触发的播放，翻译就绪或超时后自动继续；切视频、禁用或运行时重建也会解除这次暂停，避免视频被遗留在暂停状态。后台语义恢复或预热失败时继续保留已可看的 fallback，不会二次暂停或清空字幕。
...
- 按句末标点 / 最大时长（~6s）/ 最大词数（~16）重新切句；语法未完成尾部可按受限门限续接。
...
- **整 clip 一次请求**：semantic 模式一个 clip = 一次 `translateClipLines`，模型按完整语义单元编号 1:1 返回；fallback 不翻译。
