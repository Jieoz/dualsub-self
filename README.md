...
7. 缓存 namespace 升级为 `dsc-v58`，并按 `fallback` / `semantic` 分段模式隔离，避免旧 prompt、旧 cue 分段、旧折行排版或不同时间轴复用错误译文。
...
9. 英文完整句超过约 16 词时会寻找两侧均可独立翻译的自然从句边界，单屏最多 20 词；不在短语内部硬切，且不改写、不重排、不丢词。
