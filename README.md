...
当前正式版：**v0.5.11**。可从 [GitHub Releases](https://github.com/Jieoz/dualsub-self/releases/tag/v0.5.11) 下载 Chrome MV3 安装包。
...
- 若提示“API 返回 HTML 而不是 JSON”，扩展会显示脱敏后的 HTTP 状态、Content-Type、响应路径及是否发生重定向。如果响应确实落到控制台、首页或登录/WAF 页面，请检查 Base URL 是否应以 `/v1` 结尾；如果路径已经是 `/v1/chat/completions` 且响应头声称 JSON，v0.5.11 会明确提示这是网关/上游把 HTML 错标成 JSON，应检查所选模型的上游路由或稍后重试，不再误判为 Base URL 错误。
...
1. fallback 时间线先用于立即显示英文原文，语义恢复进行中不抢跑翻译；若语义恢复不适用、失败或当前 semantic clip 预热失败，v0.5.11 会自动翻译已由本地 ASR 重组器整理过的 fallback cue，避免页面永久只有英文。
...
7. 缓存 namespace 为 `dsc-v60`，并按 `fallback` / `fallback-translation` / `semantic` 分段模式隔离，避免不同时间轴或降级质量的译文相互污染。
