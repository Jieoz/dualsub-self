...
## 翻译质量：v0.5.3 语义恢复、上下文感知且严格 1:1

v0.5.3 在 v0.5.2 的上下文翻译基础上，优先为带可靠词级时间的 YouTube JSON3 自动字幕恢复自然英文句界，再保持源 cue / 译文 / 时间轴严格 1:1：

1. 每个 clip 仍把重组后的源 cue 编号后一次发给 OpenAI 兼容 `/chat/completions`；模型可读取整段上下文，但第 N 条只能表达第 N 个 cue 的信息。
2. 半句允许与相邻 cue 自然承接，不再强迫每条独立完整；禁止为了“完整”补入源文没有的信息。
...
4. 保留必要的中文逗号、句号、问号和感叹号；折叠重复尾标点、去掉标点后的异常空格，并清理 ASR 行首孤立英文句点。
5. 模型缺号或数量不符时拒绝部分结果，绝不猜填或缓存；运行层按既有退避机制整包重试。真正空响应仍暂显原文并重试。
6. 每个字幕单元继续直接沿用对应 cue 的原文与起止时间；超长译文只在同一单元内按安全短语边界换行，绝不按中文字符硬切词。
7. 缓存 namespace 升级为 `dsc-v53`，并按 `fallback` / `semantic` 分段模式隔离，避免旧 prompt、旧 cue 分段或不同时间轴复用错误译文。
8. 首屏不再等待整轨语义恢复：先安装可播放的 fallback 时间轴并启动首段翻译，语义恢复在后台完成。semantic 时间轴会先预热实时播放位置对应的译文，且每次异步等待后重新核对播放头；只有当前段已经准备好时才原子切换，避免屏上中文闪回“翻译中…”或因 seek / 跨段使用过期结果。
9. 英文长句的 24 词是优先目标，不是破坏阅读的硬切线：若严格切分会留下孤词、介词/连词起始半句或介词尾悬空，扩展会把相邻原词单元回并（最多 36 词）。这一层不请求模型、不改写、不重排，也不丢词；宁可保留一条略长但完整的句子，也不制造 `altogether` 这类单词独占一屏。
...
- **译文 1:1 对齐（v0.5.2）**：模型按编号返回与当前源单元等量的译文，每个字幕单元直接沿用对应 cue 时间轴；超长译文只在单元内部安全换行。`maxCharsPerScreen` / `maxDurPerScreen` 仍仅用于旧配置 round-trip。
- 「句间空隙」仍在 popup「字幕节奏」区可调；单屏字数/时长控件仅作旧配置兼容提示。

### 边播边翻：预取 + 并发 gate + 缓存

- **整 clip 一次请求**：v0.5.1 一个 clip = 一次 `translateClipLines`，模型按源 cue 编号 1:1 返回；不再有 clip 内分批或逐行补翻。
...
core.js         纯逻辑（解析/清洗/语义重组/cue 1:1 翻译/时间轴/clip 切分/缓存/退避），无浏览器依赖，可 Node 单测
