...
## 翻译质量：v0.5.1 上下文感知且严格 1:1

v0.5.1 在 v0.5.0 的源 cue / 译文 / 时间轴严格 1:1 基础上，重点修复半句生硬、无依据补译和标点粘连：
...
3. 英文源 cue 若以介词、连接词、冠词或助动词等语法未完成词结尾，可在受限条件下吸收下一 cue：仅一次、间隙 < 2.2s、总时长 ≤ 8s，并受扩展词数硬上限约束。
...
7. 缓存 namespace 升级为 `dsc-v51`，避免旧 prompt / 旧分段结果污染新版本。

首包默认最多等待 8 秒：扩展只会暂停自己触发的播放，翻译就绪或超时后自动继续；切视频、禁用或运行时重建也会解除这次暂停，避免视频被遗留在暂停状态。
...
- 按句末标点 / 最大时长（~6s）/ 最大词数（~16）重新切句；语法未完成尾部可按上述 v0.5.1 硬门限续接一次。
...
- **译文 1:1 对齐（v0.5.1）**：模型按编号返回与源 cue 等量的译文，每个字幕单元直接沿用对应 cue 时间轴；超长译文只在单元内部安全换行。`maxCharsPerScreen` / `maxDurPerScreen` 仍仅用于旧配置 round-trip。
...
- **缺槽失败关闭（v0.5.1）**：编号缺失或数量异常时不接受部分译文、不逐行猜填；该 clip 保持原文显示，并由统一退避调度整包重试。
