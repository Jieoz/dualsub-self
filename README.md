...
2. system prompt 明确要求模型**直接输出适合阅读的中文字幕行**：自然语义/短语边界断行、尽量 8–16 个汉字、去行尾逗号/句号、只输出中文行、不输出行号/解释。
3. 本地代码只做轻量清洗（剥漏网行号、去行尾逗号/句号、丢空行、合并连续重复行），然后按输出行的字符长度比例给整个 clip 分配时间轴。
4. 代码**绝不再按字/词切译文**；行边界由模型决定，因此从根上避免半词/半成语跨行。

为压住推理模型（如 gpt-5.x-mini）的延迟和 reasoning token，默认配置把 `clipSeconds` 调到 15 秒、`reasoningEffort` 设为 `low`，并保留 `minLineChars` 作为短行合并兜底。离线 E2E harness 的 mock 验收会专门扫描相邻字幕行，确认没有「经/常」「隔三差/五」「3,/000」这类跨行切词。
...
- **clip 按 cue 边界切**（默认目标 ~15s/clip）：绝不在句子中间断、clip 之间不重叠不重复，同时让单次 prompt 更短、延迟更稳。
