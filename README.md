...
为压住推理模型（如 gpt-5.x-mini）的延迟和 reasoning token，默认配置把 `clipSeconds` 调到 **12 秒**（v0.4.1 从 15 收到 12）、`reasoningEffort` 设为 `low`，并保留 `minLineChars` 作为短行合并兜底。离线 E2E harness 的 mock/real 验收会专门扫描相邻字幕行，确认没有「经/常」「隔三差/五」「3,/000」这类跨行切词。
