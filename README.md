...
## 翻译质量：v0.5 源 cue 与译文 1:1

v0.5.0 继续保留 v0.4 去掉“代码硬切中文”的根治方向，同时把模型输出改成与重组后的源 cue **按编号 1:1 对齐**：

1. 每个 clip 把重组后的源 cue 编号后一次发给 OpenAI 兼容 `/chat/completions`。
2. 模型必须按相同编号和行数返回；第 N 条译文只对应第 N 条原文，不跨 cue 合并、拆分、遗漏或重排。
3. 每个字幕单元直接沿用对应 cue 的原文与起止时间，避免中英文错位。
4. 中文字幕目标为自然、完整、尽量 12–20 个汉字；超长时只允许在同一字幕单元内部按安全短语边界换行，绝不按字符硬切词。
5. 缺号会保留空槽；模型整包空响应会返回空数组，由运行层显示原文并按退避策略重试。
6. v0.5 使用新的持久缓存 namespace，避免旧版可变行数缓存污染 1:1 对齐结果。
...
- **译文 1:1 对齐（v0.5.0）**：模型按编号返回与源 cue 等量的译文，每个字幕单元直接沿用对应 cue 时间轴；超长译文只在单元内部安全换行。`maxCharsPerScreen` / `maxDurPerScreen` 仍仅用于旧配置 round-trip。
- 「句间空隙」仍在 popup「字幕节奏」区可调；单屏字数/时长控件仅作旧配置兼容提示。

### 边播边翻：预取 + 并发 gate + 缓存

- **整 clip 一次请求**：v0.5.0 一个 clip = 一次 `translateClipLines`，模型按源 cue 编号 1:1 返回；不再有 clip 内分批或逐行补翻。
